...
static void add_row(GtkGrid *table, int row, GtkSizeGroup *size_group, const char *label_text, const char **options)
{
	GtkWidget *combo_box;
	GtkWidget *label;
...
static void toggle_grouping(GtkToggleButton *check_button, GtkSizeGroup *size_group)
{
...
GtkWidget * do_sizegroup(GtkWidget *do_widget)
{
	...
	if (!window)
	{
		window = gtk_dialog_new_with_buttons("GtkSizeGroup", GTK_WINDOW(do_widget), 0, _("_Close"), GTK_RESPONSE_NONE,
		NULL);
		...
		g_signal_connect(check_button, "toggled", G_CALLBACK (toggle_grouping), size_group);
	}
