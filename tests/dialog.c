...
static void dialog_of_user_db_name(GtkButton *button, gpointer user_data)
{

//	real_time_window;

	GtkWidget * dialog = gtk_dialog_new_with_buttons("Interactive Dialog",
	...
	GtkWidget * label = gtk_label_new_with_mnemonic("_Entry 1");
...
GtkWidget * dialog(GtkWidget *do_widget)
{
	GtkWidget *frame;
	GtkWidget *vbox;
	GtkWidget *vbox2;
	...
	if (!window)
	{
		...
		g_signal_connect(button, "clicked", G_CALLBACK (interactive_dialog_clicked), NULL);
