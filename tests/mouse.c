...
const gchar *colors[] =
{"black", "orchid", "fuchsia", "indigo", "thistle", "sienna", "azure", "plum", "lime", "navy", "maroon", "burlywood"};
...
axes_info_new(void)
{
	...
	info->axes = g_hash_table_new_full(NULL, NULL, NULL, (GDestroyNotify) g_free);
...
static void axes_info_free(AxesInfo *info)
{
	g_hash_table_destroy(info->axes);
	g_free(info);
}

static gboolean axes_info_lookup(AxesInfo *info, const gchar *axis_label, gdouble *value)
{
	gdouble *val;
	GdkAtom atom;
...
event_data_new(void)
{
	...
	data->touch_info = g_hash_table_new_full(NULL, NULL, NULL, (GDestroyNotify) axes_info_free);
...
static void update_axes_from_event(GdkEvent *event, EventData *data)
{
	...
	if (event->type == GDK_TOUCH_BEGIN || event->type == GDK_TOUCH_UPDATE || event->type == GDK_MOTION_NOTIFY
			|| event->type == GDK_BUTTON_PRESS || event->type == GDK_BUTTON_RELEASE)
	{
		...
		for (l = axes; l; l = l->next)
		{
			...
			if (!gdk_device_get_axis_value(device, event->motion.axes, l->data, &value))
				continue;
...
static gboolean event_cb(GtkWidget *widget, GdkEvent *event, gpointer user_data)
{
...
static void render_arrow(cairo_t *cr, gdouble x_diff, gdouble y_diff, const gchar *label)
{
	cairo_save(cr);
...
static void draw_axes_info(cairo_t *cr, AxesInfo *info, GtkAllocation *allocation)
{
	...
	if (axes_info_lookup(info, "Abs Tilt X", &tilt_x) && axes_info_lookup(info, "Abs Tilt Y", &tilt_y))
...
static void draw_device_info(GtkWidget *widget, cairo_t *cr, GdkEventSequence *sequence, gint *y, AxesInfo *info)
{
	PangoLayout *layout;
	GString *string;
	gint height;
	...
	g_string_append_printf(string, "Source: %s", gdk_device_get_name(info->last_source));

	if (sequence)
		g_string_append_printf(string, "\nSequence: %d", GPOINTER_TO_UINT(sequence));
...
do_event_axes(GtkWidget *toplevel)
{
	...
	if (!window)
	{
		...
		g_object_set_data_full(G_OBJECT(box), "gtk-demo-event-data", event_data, (GDestroyNotify) event_data_free);
